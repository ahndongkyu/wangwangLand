-- 봉사 단체명을 신청자 이름과 분리해 선택 항목으로 저장한다.

alter table volunteer_applications
  add column if not exists group_name text;

comment on column volunteer_applications.group_name is
  '단체 신청 시 단체명 (선택). 빈값, X, 없음은 null로 저장';

-- 기존의 `단체명 / 인솔자명` 데이터를 새 컬럼으로 분리한다.
with legacy_group_applications as (
  select
    id,
    trim(substring(applicant_name from 1 for strpos(applicant_name, ' / ') - 1))
      as legacy_group_name,
    trim(substring(applicant_name from strpos(applicant_name, ' / ') + 3))
      as legacy_applicant_name
  from volunteer_applications
  where party_size > 1
    and group_name is null
    and strpos(applicant_name, ' / ') > 0
    and nullif(
      trim(substring(applicant_name from strpos(applicant_name, ' / ') + 3)),
      ''
    ) is not null
)
update volunteer_applications as application
set
  group_name = case
    when legacy.legacy_group_name = '' then null
    when lower(legacy.legacy_group_name) = 'x' then null
    when legacy.legacy_group_name = '없음' then null
    else legacy.legacy_group_name
  end,
  applicant_name = legacy.legacy_applicant_name
from legacy_group_applications as legacy
where application.id = legacy.id;

-- 승인 시 생성되는 캘린더 제목도 단체명이 있을 때만 포함한다.
...
begin
  ...
  update volunteer_applications
     set status = p_status,
         ...
         approved_by = case
           when p_status = '승인'
             and v_app.status not in ('승인', '일정변경요청')
             then p_created_by
           else approved_by
         end,
         approved_at = case
  ...
  v_display_name := concat_ws(
    ' / ',
    nullif(trim(v_app.group_name), ''),
    v_app.applicant_name
  );
  v_title := case
    when coalesce(v_app.party_size, 1) > 1
      then v_display_name || ' 외 ' || (v_app.party_size - 1)::text || '명'
    else v_display_name
  end;
  ...
  if p_schedule_action = 'replace' then
    update volunteer_applications
       set available_dates = array(
             select (starts_at at time zone 'Asia/Seoul')::date
