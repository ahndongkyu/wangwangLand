-- 봉사 신청 승인 담당자와 승인 시각을 관리자용 감사 정보로 기록한다.

alter table volunteer_applications
  add column if not exists approved_by uuid references profiles(id) on delete set null,
  add column if not exists approved_at timestamptz;

create index if not exists idx_volunteer_applications_approved_by
  on volunteer_applications(approved_by)
  where approved_by is not null;

comment on column volunteer_applications.approved_by is
  '봉사 신청을 승인한 운영진 profiles.id. 관리자 화면에서만 표시.';
comment on column volunteer_applications.approved_at is
  '봉사 신청 승인 시각. 일정변경 승인 시에는 기존 승인 기록을 유지.';
