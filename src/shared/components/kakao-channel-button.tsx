...
export function KakaoChannelButton() {
  ...
  return (
    ...
    >
      <div
        id="channel-quick-links"
        aria-hidden={!expanded}
        className={expanded ? "flex flex-col items-end gap-2.5" : "hidden"}
      >
          {/* 카카오 */}
          <FloatBtn
      ...
      <FloatBtn
        label={expanded ? "채널 바로가기 닫기" : "채널 바로가기 펼치기"}
        onClick={() => setExpanded((current) => !current)}
        expanded={expanded}
        controls="channel-quick-links"
        className="flex size-12 items-center justify-center rounded-full bg-[#2A3D2F] text-white shadow-[0_3px_12px_rgba(0,0,0,0.16)] transition-all hover:-translate-y-0.5 hover:shadow-[0_5px_16px_rgba(0,0,0,0.2)] dark:bg-[#F5EDE0] dark:text-[#2B2520]"
      >
        {expanded ? <X className="size-5" /> : <Share2 className="size-5" />}
      </FloatBtn>
