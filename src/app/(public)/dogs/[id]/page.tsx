...
export async function generateMetadata({
  ...
  return {
    title: `${dog.name}`,
    description:
      dog.description ?? `${dog.name}의 프로필 페이지입니다.`,
  }
}
...
export default async function DogDetailPage({
  ...
  const images = dog.images.length > 0 ? dog.images : []
  const mainImage = images[dog.thumbnail_index] ?? images[0] ?? null
  const otherImages = images.filter((_, idx) => idx !== dog.thumbnail_index)
